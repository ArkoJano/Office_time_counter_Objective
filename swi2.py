...
def add_all_days_to_collection(input_list, colleciton_of_days):
    ...
    '''
    funkcja obiekty OneDay dla kazdego indywidualnego
    dnia i dodaje go do kolekcji dni
    '''
...
class CollectionOfDays():
    ...
    def is_this_correct_time(self, now, before):
        ...
        if now_date == before_date:
            ...
            if now_td < before_td:
                self.dict_of_all_days[now_date].set_nv_inconclusive()
    ...
    def add_entry_hour(self, formated_day):
        '''
        metoda dodaje do konkretnego dnia
    ...
    def setting_flags(self, day):
        ...
        if (day.weekday() == 5 or day.weekday() == 6):      # jesli dany dzien byl  
            self.dict_of_all_days[day].flags['weekend'] = "w "     # weekendem - dodaj flage 'w'
        
        if (self.dict_of_all_days[day].sum_of_work > timedelta(hours=9)):   # jesli przepracowal ponad 9h 
            self.dict_of_all_days[day].flags['overtime'] = "ot "               # dodaj flage 'ot'
        
        if (self.dict_of_all_days[day].sum_of_work < timedelta(hours=6)):   # jesli przepracowal mniej niz
            self.dict_of_all_days[day].flags['undertime'] = "ut "              # 6h - dodaj flage 'ut'

        if self.dict_of_all_days[day].flags['inconclusive_nv'] > 0:    # jesli godziny nie byly zapisane
            self.dict_of_all_days[day].flags['inconclusive'] = 'i '    # w pliku "chronologicznie"
    ...
    def write_data_to_result(self):
        ...
        with open(OUTPUT, 'w') as result:
            ...
            for day in self.dict_of_all_days.keys():
                ...
                weekend = self.dict_of_all_days[day].flags['weekend']
                overtime = self.dict_of_all_days[day].flags['overtime']
                undertime = self.dict_of_all_days[day].flags['undertime']
                inconclusive = self.dict_of_all_days[day].flags['inconclusive']
                ...
                print(f"Day {day} Work {time_of_work} {weekend}{overtime}{undertime}{inconclusive} {self.weekly_time_of_work} {self.time_under_over}")
